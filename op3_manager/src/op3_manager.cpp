...
const int USER_LED_CTRL_TABLE = 25; // MODIFICADO: Añadimos la dirección de los LEDs de usuario
const int TORQUE_ON_CTRL_TABLE = 40;
...
int main(int argc, char **argv)
{
  ...
  if (g_is_simulation == false)
  {
    ...
    // MODIFICADO: Cambiado de LED RGB a LEDs de Usuario
    int user_led_value = 7; // 7 = 1 (USER3) + 2 (USER4) + 4 (STATUS)
    int _return = packet_handler->write1ByteTxRx(port_handler, SUB_CONTROLLER_ID, USER_LED_CTRL_TABLE, user_led_value);

    if(_return != 0)
      RCLCPP_ERROR(node->get_logger(), "Fail to control USER LEDs [%s]", packet_handler->getRxPacketError(_return));
    else
      RCLCPP_INFO(node->get_logger(), "USER LEDs (3, 4, STATUS) are ON!");
  ...
  if (g_robot_file == "")
  {
    ...
    if (g_init_file == "")
  {
    RCLCPP_ERROR(node->get_logger(), "NO init file path in the ROS parameters.");
    return -1;
  }
